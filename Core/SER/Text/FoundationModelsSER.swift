...
public actor FoundationModelsSER: TextSER {
    ...
    /// Human-readable language label rendered into the opening line of
    /// the prompt — e.g. "Japanese", "English". Nil = language-agnostic
    /// opener, which still works (Apple FM is multilingual) but provides
    /// a small nudge toward the expected register when set. Mutated via
    /// `setLanguage(_:)` so the pipeline can re-target between sessions
    /// without recreating the actor.
    private var languageLabel: String?

    public init(languageLabel: String? = nil) {
        self.languageLabel = languageLabel
    }

    public func setLanguage(_ label: String?) {
        languageLabel = label
    }
    ...
    private var instructions: String {
        ...
        if let languageLabel {
            opener = "You are a \(languageLabel)-language affect annotator."
        } else {
            opener = "You are an affect annotator."
        }
        return opener + "\n\n" + Self.instructionsBody
    }
